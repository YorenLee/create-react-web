    ...
    return (
        <section className={styles.group} style={{ backgroundColor: bgColor }}>
            <div className={styles.groupInner}>
                {/* 左侧 sticky 图片区 — 纯 CSS 驱动切换 */}
                <div className={styles.stickyLeft}>
                    <div className={styles.imageContainer}>
                        {group.images.map((src, i) => (
                            <img key={src} src={src} alt={`group-${groupIndex}-img-${i}`} className={styles.image} />
                        ))}
...
const ScrollytellingCSS: React.FC = () => {
    return (
        <div className={styles.container}>
            <header className={styles.hero}>
                <h1 className={styles.heroTitle}>滚动叙事</h1>
                <p className={styles.heroSubtitle}>纯 CSS scroll-timeline 驱动 · 零 JS 动画逻辑</p>
                <p className={styles.heroBadge}>scroll-timeline</p>
...
export default ScrollytellingCSS;
