...
function ScrollytellingGroup({ group, groupIndex }: { group: StoryGroup; groupIndex: number }) {
    ...
    useEffect(() => {
        const updateOnScroll = () => {
            ...
            chapterRefs.current.forEach((chapterEl, index) => {
                if (!chapterEl) return;

                const rect = chapterEl.getBoundingClientRect();
                // progress: 0 = 章节底部刚到视窗底部, 1 = 章节顶部刚离开视窗顶部
                const progress = clamp((vh - rect.top) / (vh + rect.height), 0, 1);

                // === 图片动画 ===
                const imgEl = imageRefs.current[index];
                if (imgEl) {
                    // 将 progress 映射到图片的 fadeIn 区间
                    // 淡入：progress 0.15 ~ 0.4 → opacity 0 ~ 1
                    // 保持：progress 0.4 ~ 0.6 → opacity 1
                    // 淡出：progress 0.6 ~ 0.85 → opacity 1 ~ 0
                    let opacity: number;
                    if (index === 0) {
                        // 第一张：初始可见，后半段淡出
                        opacity = progress < 0.6 ? 1 : 1 - clamp((progress - 0.6) / 0.25, 0, 1);
                    } else if (index === group.images.length - 1) {
                        // 最后一张：淡入后保持
                        opacity = clamp((progress - 0.15) / 0.25, 0, 1);
                    } else {
                        // 中间张：淡入 → 保持 → 淡出
                        const fadeIn = clamp((progress - 0.15) / 0.25, 0, 1);
                        const fadeOut = 1 - clamp((progress - 0.6) / 0.25, 0, 1);
                        opacity = Math.min(fadeIn, fadeOut);
                    }

                    const scale = lerp(1.15, 1, clamp(progress / 0.5, 0, 1));
                    const translateY = lerp(20, 0, clamp(progress / 0.4, 0, 1));
                    const blur = lerp(4, 0, clamp((progress - 0.1) / 0.2, 0, 1));
                ...
                if (contentEl) {
                    const textOpacity = clamp((progress - 0.2) / 0.25, 0.1, 1);
                    const textY = lerp(50, 0, clamp((progress - 0.15) / 0.3, 0, 1));
                    const textBlur = lerp(6, 0, clamp((progress - 0.15) / 0.2, 0, 1));

                    contentEl.style.opacity = String(textOpacity);
                    contentEl.style.transform = `translateY(${textY}px)`;
                    contentEl.style.filter = `blur(${textBlur}px)`;
                }
            });
        };
            ...
            <div className={styles.groupInner}>
                ...
                <div className={styles.narrativeRight}>
                    {group.chapters.map((chapter, i) => (
                        <div key={i} ref={setChapterRef(i)} className={styles.chapter}>
                            <div ref={setContentRef(i)} className={styles.chapterContent}>
...
const ScrollytellingScroll: React.FC = () => {
    return (
        <div className={styles.container}>
            <header className={styles.hero}>
                <h1 className={styles.heroTitle}>滚动叙事</h1>
                <p className={styles.heroSubtitle}>scroll 事件 + 连续进度驱动 · 丰富图片动画</p>
                <p className={styles.heroBadge}>scroll progress</p>
...
export default ScrollytellingScroll;
